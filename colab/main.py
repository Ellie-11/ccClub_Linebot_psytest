...
url = 'https://ccclub-linebot-test-default-rtdb.firebaseio.com/'    # 我的Firebase Realtime database URL
...
handler = WebhookHandler('e0ee7e3d6f78c0fffa24dd926c683887')   # 確認 token 是否正確
...
def linebot():
    ...
    try:
      signature = request.headers['X-Line-Signature']          # 確認 secret 是否正確
      handler.handle(body, signature)                   # 加入回傳的 headers
      tk = json_data['events'][0]['replyToken']             # 取得回傳訊息的 Token
      msg = json_data['events'][0]['message']['text']          # 取得 LINE 收到的文字訊息
    except Exception as e:                        # 輸出詳細的錯誤訊息
      print('Error:', e)
    return 'OK'

@handler.add(MessageEvent, message=TextMessage)                # 處理回覆訊息的函式
def handle_message(event):
    global points #引用全局變量
    global got_it
    global result
    message_text = event.message.text
    print(event)                              # 印出要回傳的 event 訊息
    ...
    elif message_text == '算了，太晚我就不要了':
      points = 0 # 重置測驗分數
      got_it = False
      result = 0
      reply_bye_array = []                     # 回覆使用者拒絕的訊息陣列
      reply_bye_array.append( TextSendMessage(text='残念ですね。。。') )  # 回覆測驗結果訊息(description)
      reply_bye_array.append( ImageSendMessage(original_content_url = 'https://www.niusnews.com/upload/posts/po5_29953_1421316750.jpg',
      preview_image_url = 'https://www.niusnews.com/upload/posts/po5_29953_1421316750.jpg')) # 皮卡丘哭哭圖
      line_bot_api.reply_message(event.reply_token,reply_bye_array)

    elif message_text == 'Got it! 開始吧！':
      if not got_it:
        got_it = True
        question1()
      else:
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text='請從 Q1 開始回答哦！'))

    elif message_text in ['A. 海邊', 'B. 西瓜', 'C. 啤酒', 'D. 剉冰']:    # Q1 答案
      if got_it:
          fdb.put('/', 'Q1', 70 - ord(message_text[0]) - 1) # 轉換代號為分數，並以同步新增，在節點Q1紀錄分數
          question2()                          # 收到答案後，提出下一題
          # 輸出資料庫內容檢查
          snapshot = fdb.get('/', 'Q1')
          print(snapshot)

    elif message_text in ['A. 冰可樂/氣泡水', 'B. 冰咖啡', 'C. 微冰無糖茶飲', 'D. 沁涼白開水']: # Q2 答案
      if got_it:               
          fdb.put('/', 'Q2', 70 - ord(message_text[0]) - 1) # 轉換代號為分數，並以同步新增，在節點Q2紀錄分數
          question3()                          # 收到答案後，提出下一題                       
          # 輸出資料庫內容檢查
          snapshot = fdb.get('/', 'Q2')
          print(snapshot)
    
    elif message_text in ['A. 催狂魔', 'B. 一隻烏龜', 'C. 是鯨魚', 'D. 就是一片雲該有的形狀']: # Q3 答案
      if got_it:               
          fdb.put('/', 'Q3', 70 - ord(message_text[0]) - 1) # 轉換代號為分數，並以同步新增，在節點Q3紀錄分數
          question4()                          # 收到答案後，提出下一題
          # 輸出資料庫內容檢查
          snapshot = fdb.get('/', 'Q3')
          print(snapshot)

    elif message_text in ['A. 妮妮媽媽的兔兔', 'B. 社畜以公司為家需要拖鞋', 'C. 舒適午睡枕/毛毯', 'D. 提醒喝水大水壺']: # Q4 答案
      if got_it:               
          fdb.put('/', 'Q4', 70 - ord(message_text[0]) - 1) # 轉換代號為分數，並以同步新增，在節點Q4紀錄分數
          question5()                          # 收到答案後，提出下一題
          # 輸出資料庫內容檢查
          snapshot = fdb.get('/', 'Q4')
          print(snapshot)

    elif message_text in ['A. 台味好!獨立搖滾聽團仔', 'B. Kpop韓團系列', 'C. 英文歌曲pop music', 'D. 爵士樂 不死']:  # Q5 答案
      if got_it:
        fdb.put('/', 'Q5', 70 - ord(message_text[0]) - 1) # 轉換代號為分數，並以同步新增，在節點Q5紀錄分數
        # 輸出資料庫內容檢查
        snapshot = fdb.get('/', 'Q5')
        print(snapshot)
        # 以字典格式輸出所有答案
        ...
        line_bot_api.reply_message(event.reply_token, reply_array)  # 以陣列回覆訊息
